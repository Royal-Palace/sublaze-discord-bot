	...
	async run(message) {
		...
		if (!message.channel.guild)
			return message.channel.send(
				"I can't execute commands inside DMs! Please run this command in a server."
			);
		...
		if (prefix === process.env.PREFIX && !process.env.PREFIX.endsWith(" ")) {
			...
			prefix === messageHelper.server.prefix &&
			!messageHelper.server.prefix.endsWith(" ")
		) {
			args = messageContent.split(" ");
			command = args.shift().toLowerCase();
		...
		if (process.env.NODE_ENV == "production")
			Statcord.ShardingClient.postCommand(commandName, ctx.author.id, this.client);
		...
		if (cmd.permissions.userPermissions.length > 0) {
			...
			if (missingPerms.length > 0) {
				return message.channel.send(
					`You don't have the required permissions to execute this command. Missing permission(s): **${missingPerms.join(
						", "
					)}**`
				);
			}
		...
		if (!message.guild && cmd.guildOnly)
			return message.channel.send(
				"I can't execute that command inside DMs!. Please run this command in a server."
			);

		if (cmd.voiceRequirements.isInVoiceChannel && !message.member.voice.channel)
			return messageHelper.sendResponse("noVoiceChannel");
		else if (
			cmd.voiceRequirements.isInSameVoiceChannel &&
			message.guild.members.me.voice.channel &&
			!message.guild.members.me.voice.channel.equals(message.member.voice.channel)
		)
			return messageHelper.sendResponse("sameVoiceChannel");
		else if (cmd.voiceRequirements.isPlaying && !this.client.music.players.get(message.guild.id))
		...
		if (prefix == process.env.PREFIX) {
			if (
				!args[0] &&
				cmd.args === true &&
				((cmd.acceptsAttachments && message.attachments.size == 0) || !cmd.acceptsAttachments)
			) {
