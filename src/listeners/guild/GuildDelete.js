		...
		try {
			this.client.shard.fetchClientValues("guilds.cache.size").then((guilds) => {
				...
				const embed = new EmbedBuilder()
					.setDescription(
						`${this.client.config.emojis.removed} Ear Tensifier has been removed from a server.`
					)
				...
				if (this.client.earTensifiers.includes(this.client.user.id))
					this.client.shard.broadcastEval(shardMessage, {
						context: {
							channel: this.client.config.channels.guildlogs,
							embed: embed,
							isShard: true,
						},
					});
