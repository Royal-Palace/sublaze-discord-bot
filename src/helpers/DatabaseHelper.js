import Bot, { findById } from "../models/Bot";
import User, { findById as _findById } from "../models/User";
import { findById as __findById } from "../models/Server";
...
export default class DatabaseHelper {
	...
	updateAll() {
		if (Date.now() > this.client.lastUpdatedDatabase + 1200000) {
			const updateBot = new Promise((resolve) => {
				findById(this.client.user.id).then(async (bot) => {
					if (!bot) {
						...
						bot
							.updateOne({ commandsUsed: bot.commandsUsed + this.client.totalCommandsUsed })
							.catch((e) => this.logger.error(e));
						bot
							.updateOne({ songsPlayed: bot.songsPlayed + this.client.totalSongsPlayed })
							.catch((e) => this.logger.error(e));
					}
					...
					for (let i = 0; i < this.client.timesCommandsUsed.length; i++) {
						...
						if (commandIndex === -1) {
							bot
								.updateOne({ commands: [...bot.commands, { _id: cmd.name, timesUsed: 1 }] })
								.catch((e) => this.logger.error(e));
						} else {
							bot
								.updateOne({
									commands: [
										...bot.commands.slice(0, commandIndex),
										{
											_id: cmd.name,
											timesUsed: bot.commands[commandIndex].timesUsed + cmd.timesUsed,
										},
										...bot.commands.slice(commandIndex + 1),
									],
								})
								.catch((e) => this.logger.error(e));
						}
			...
			const updateUsers = new Promise((resolve) => {
				for (let i = 0; i < this.client.usersStats.length; i++) {
					...
					_findById(user.id).then(async (u) => {
						if (!u) {
							...
							u.updateOne({
								songsPlayed: u.songsPlayed + user.songsPlayed,
								commandsUsed: u.commandsUsed + user.commandsUsed,
							}).catch((e) => this.logger.error(e));
						}
	...
	static async getDefaultVolume(server) {
		return (await __findById(server.id)).defaults.volume;
	}

	static async shouldSendNowPlayingMessage(server) {
		return (await __findById(server.id)).nowPlayingMessages;
	}
}
